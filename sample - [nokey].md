...
================================================================================
🎛️ WINDOWS TITLE BAR CUSTOMIZATION & DWM REPAINT OVERRIDE
================================================================================

To dynamically control the native Windows title bar theme (Light/Dark mode) in your Flutter desktop application, overriding OS constraints and avoiding repaint lag:

1. **Native C++ MethodChannel Registration** (`windows/runner/flutter_window.cpp`):
   Include `<dwmapi.h>`, `<flutter/method_channel.h>`, and `<flutter/standard_method_codec.h>`. Register a method channel inside `FlutterWindow::OnCreate` right after registering plugins:

```cpp
// Add to includes at the top of windows/runner/flutter_window.cpp:
#include <dwmapi.h>
#include <flutter/method_channel.h>
#include <flutter/standard_method_codec.h>

// Add inside FlutterWindow::OnCreate in windows/runner/flutter_window.cpp:
    ...
    [this](const flutter::MethodCall<flutter::EncodableValue>& call,
      ...
      if (call.method_name().compare("setTheme") == 0) {
        ...
        if (arguments && std::holds_alternative<std::string>(*arguments)) {
          ...
          HWND hWnd = GetHandle();
          if (hWnd) {
            // Apply both 19 (older Win10) and 20 (Win11/modern Win10) for maximum compatibility
            ...
            // Force immediate DWM frame recalculation by toggling window size by 1 pixel
            RECT rect;
            ...
            // Force titlebar caption repaint immediately by simulating focus messages
...
2. **Invoke MethodChannel in Dart Theme Manager** (e.g., `lib/modules/ui/styles.dart`):
   Whenever your theme notifier updates, call the native channel to force the title bar to redraw to the correct style:

```dart
import 'package:flutter/services.dart';

// Inside your ThemeNotifier or Theme Toggle handler:
const MethodChannel('ja_translate/theme')
    .invokeMethod('setTheme', isDark ? 'dark' : 'light')
    .catchError((_) {});
```

3. **Handle Windows 10 Border Radius Clip Artifacts**:
   On Windows 10, the OS window manager does not support rounded frames natively, showing black corner pixels under client rounding. Set the corner radius to zero on Windows 10:

```dart
final bool useRoundCorners = !Platform.isWindows || isWindows11OrNewer();
final borderRadius = useRoundCorners ? BorderRadius.circular(15) : BorderRadius.zero;
```

================================================================================
🪟 WINDOWS 10 COMPATIBILITY: GLASSMORPHISM & TINT CONTRAST
================================================================================

When building transparent/glassmorphic (acrylic/mica) desktop applications using packages like `flutter_acrylic` and `window_manager`, Windows 10 does not render background blur natively in the same way as Windows 11. Instead, it often falls back to a solid dark grey/black background layer. 

To ensure the interface remains beautiful, readable, and high-contrast on Windows 10:

1. **Avoid `Colors.transparent` on Scaffold**: Always paint `backgroundColor: _c.bgPrimary` on your `Scaffold` instead of leaving it fully transparent. This ensures that the primary theme background color (e.g., bright slate-white in light mode or dark-slate in dark mode) is drawn over the solid window background layer.
2. **Opacity Fallbacks for Solids**: Avoid hardcoding static `.withOpacity(...)` values on container backgrounds (like sidebars or cards). Instead, check `AppConfig.enableTransparency` dynamically:
   - If transparency is active: draw semi-transparent background (e.g. `_c.bgSecondary.withOpacity(0.4)`).
   - If transparency is disabled: draw fully opaque background (`_c.bgSecondary`).
3. **Detection & Defaulting**: In your configuration manager, default `enableTransparency` to `false` on Windows 10, and only default to `true` on Windows 11 and newer:
